...
func (ar *AddressRepository) AddAddress(address domain.Address) (domain.Address, error) {
	err := ar.DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Exec(`
		INSERT INTO addresses (street, city, state, district, zip_code, country)
		VALUES(?, ?, ?, ?, ?, ?)
		`, , address.Street, address.City, address.State, address.District, address.ZipCode, address.Country).Error; err != nil {
			return err
		}
		if err := tx.Raw(`
