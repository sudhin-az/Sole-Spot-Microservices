...
func (pr *ProductRepository) ListProducts() ([]models.ProductBrief, error) {
	...
	err := pr.DB.Raw("SELECT id, name, description, category_id, size, stock, price FROM products").Scan(&product).Error
	if err != nil {
		return nil, err
	}
	return product, nil
...
func (pr *ProductRepository) UpdateProducts(product models.Product) (domain.Product, error) {
	...
	// Update with incoming product values
	err := pr.DB.Model(&domain.Product{}).Where("id = ?", product.ID).Updates(domain.Product{
		Name:        product.Name,
	...
	err = pr.DB.First(&pro, product.ID).Error
	if err != nil {
		return domain.Product{}, err
	}
