...
func (or *OrderRepository) GetOrderDetails(userID int) ([]models.FullOrderDetails, error) {
	...
	for _, order := range orders {
		...
		Where("order_id = ?", order.OrderId).Scan(&products).Error
		if err != nil {
			return nil, err
		}

		fullOrders = append(fullOrders, models.FullOrderDetails{
