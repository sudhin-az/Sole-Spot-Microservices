...
func (u *userRepository) UserSignUp(user models.UserSignUp) (models.UserDetails, error) {
	...
	err := u.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Raw(`
		SELECT id, first_name, last_name, email, phone FROM users WHERE email = ?
...
func (u *userRepository) UserLogin(user models.UserLogin) (models.UserDetails, error) {
	var userDetails models.UserDetails
	query := `SELECT id, first_name, last_name, email, password, phone FROM users WHERE email = ?`
