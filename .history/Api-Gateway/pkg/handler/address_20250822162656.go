...
func (ad *AddressHandler) AddAddress(c *gin.Context) {
	...
	if !exists {
		errs := response.ClientResponse(http.StatusUnauthorized, "User not authorized", nil, nil)
		c.JSON(http.StatusUnauthorized, errs)
		return
	}

	addressDetail.UserID = uint(userID.(int))
...
func (ad *AddressHandler) GetAddressByID(c *gin.Context) {
	id := strings.TrimSpace(c.q)
	...
	address, err := ad.GRPC_Client.GetAddressByID(uint(addressID))
	if err != nil {
...
func (ad *AddressHandler) UpdateAddress(c *gin.Context) {
	...
	if err != nil {
		errs := response.ClientResponse(http.StatusBadRequest, "Invalid ID", nil, err.Error())
		c.JSON(http.StatusBadRequest, errs)
		return
	}

	addressDetail.ID = uint(id)
