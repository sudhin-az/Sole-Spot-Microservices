...
func (pu *ProductUseCase) AddProducts(product models.Product) (domain.Product, error) {
	exist := pu.ProductRepository.ProductAlreadyExist(product.Name)
	if exist {
		return domain.Product{}, errors.New("product already exits")
	}
	productResponse, err := pu.ProductRepository.AddProducts(product)
	if err != nil {
...
func (pu *ProductUseCase) UpdateProduct(product models.Product) (domain.Product, error) {
